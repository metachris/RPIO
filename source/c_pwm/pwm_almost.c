...
static void set_servo(int channel, int width);
...
shutdown()
{
    ...
    for (i = 0; i < MAX_GPIOS; i++) {
        if (ctl_arr[i].dma_reg && ctl_arr[i].virtbase) {
            printf("shutdown channel %d\n", i);
            set_servo(i, 0);
            udelay(PERIOD_TIME_US);
            ctl_arr[i].dma_reg[DMA_CS] = DMA_RESET;
            udelay(10);
        }
    }
}
...
{
    uint32_t offset = (uint8_t *)virt - ctl_arr[channel].virtbase;
    return ctl_arr[channel].page_map[offset >> PAGE_SHIFT].physaddr + (offset % PAGE_SIZE);
}
...
uint8_t* get_cb(int channel) {
    return ctl_arr[channel].virtbase + (sizeof(uint32_t) * NUM_SAMPLES);
}

// Set one servo to a specific pulse
static void
set_servo(int channel, int width)
{
    int i;
    ...
    uint32_t *dp = (uint32_t *) ctl_arr[channel].virtbase;
...
make_pagemap(int channel)
{
    ...
    ctl_arr[channel].page_map = malloc(NUM_PAGES * sizeof(*ctl_arr[channel].page_map));

    if (ctl_arr[channel].page_map == 0)
    ...
    if (lseek(fd, (uint32_t)ctl_arr[channel].virtbase >> 9, SEEK_SET) !=
                        (uint32_t)ctl_arr[channel].virtbase >> 9) {
        fatal("rpio-pwm: Failed to seek on %s: %m\n", pagemap_fn);
    }
    for (i = 0; i < NUM_PAGES; i++) {
        uint64_t pfn;
        ctl_arr[channel].page_map[i].virtaddr = ctl_arr[channel].virtbase + i * PAGE_SIZE;
        // Following line forces page to be allocated
        ctl_arr[channel].page_map[i].virtaddr[0] = 0;
        if (read(fd, &pfn, sizeof(pfn)) != sizeof(pfn))
            fatal("rpio-pwm: Failed to read %s: %m\n", pagemap_fn);
        if (((pfn >> 55) & 0x1bf) != 0x10c)
            fatal("rpio-pwm: Page %d not present (pfn 0x%016llx)\n", i, pfn);
        ctl_arr[channel].page_map[i].physaddr = (uint32_t)pfn << PAGE_SHIFT | 0x40000000;
    }
    close(fd);
    close(memfd);
}
...
init_virtbase(int channel) {
    ctl_arr[channel].virtbase = mmap(NULL, NUM_PAGES * PAGE_SIZE, PROT_READ|PROT_WRITE,
            MAP_SHARED|MAP_ANONYMOUS|MAP_NORESERVE|MAP_LOCKED, -1, 0);
    if (ctl_arr[channel].virtbase == MAP_FAILED)
        fatal("rpio-pwm: Failed to mmap physical pages: %m\n");
    if ((unsigned long)ctl_arr[channel].virtbase & (PAGE_SIZE-1))
...
{
    dma_cb_t *cbp = (dma_cb_t *) get_cb(channel);
    uint32_t *sample = (uint32_t *) ctl_arr[channel].virtbase;

    uint32_t phys_fifo_addr;
    uint32_t phys_gpclr0 = 0x7e200000 + 0x28;
    int i;

    ctl_arr[channel].dma_reg = map_peripheral(DMA_BASE, DMA_LEN) + (DMA_CHANNEL_INC * channel);
    ...
    memset(sample, 0, sizeof(NUM_SAMPLES * sizeof(uint32_t)));
    ...
    ctl_arr[channel].dma_reg[DMA_CS] = DMA_RESET; // DMA channel reset
    udelay(10);
    ctl_arr[channel].dma_reg[DMA_CS] = DMA_INT | DMA_END; // Interrupt status & DMA end flag
    ctl_arr[channel].dma_reg[DMA_CONBLK_AD] = mem_virt_to_phys(channel, get_cb(channel));  // initial CB
    ctl_arr[channel].dma_reg[DMA_DEBUG] = 7; // clear debug error flags
    ctl_arr[channel].dma_reg[DMA_CS] = 0x10880001;    // go, mid priority, wait for outstanding writes
}

// Takes care of initializing one channel (virtbase, pagemap, ctrl_data)
static void
init_channel(int channel)
{
